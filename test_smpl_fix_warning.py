...
class SMPL(pl.LightningModule):
    def __init__(self, model_path, obj_saveable=False):
        ...
        if obj_saveable:
            self.faces = model_params['f']
        else:
            self.faces = None

        np_v_template = np.array(model_params['v_template'], dtype=float)
        self.register_buffer('v_template', torch.from_numpy(np_v_template).float())
        self.size = [np_v_template.shape[0], 3]
        np_shapedirs = np.array(model_params['shapedirs'], dtype=float)
        self.num_betas = np_shapedirs.shape[-1]
        np_shapedirs = np.reshape(np_shapedirs, [-1, self.num_betas]).T
        self.register_buffer('shapedirs', torch.from_numpy(np_shapedirs).float())

        np_J_regressor = np.array(model_params['J_regressor'], dtype=float)
        self.register_buffer('J_regressor', torch.from_numpy(np_J_regressor).float())

        np_posedirs = np.array(model_params['posedirs'], dtype=float)
        num_pose_basis = np_posedirs.shape[-1]
        np_posedirs = np.reshape(np_posedirs, [-1, num_pose_basis]).T
        self.register_buffer('posedirs', torch.from_numpy(np_posedirs).float())

        self.parents = np.array(model_params['kintree_table'])[0].astype(np.int32)

        np_weights = np.array(model_params['weights'], dtype=float)
    ...
    def forward(self, shape, pose, get_skin=False):
...
if __name__ == '__main__':
    ...
    shape = np.array([-0.25349993, 0.25009069, 0.21440795, 0.78280628, 0.08625954,
                      0.28128183, 0.06626327, -0.26495767, 0.09009246, 0.06537955], dtype=float)

    shape_input = torch.tensor(np.array([shape, shape, shape])).float().to(device)
    pose_input = torch.tensor(np.array([pose,pose,pose])).float().to(device)

    verts, kpts_3d, Rs = smpl(shape_input, pose_input, get_skin=True)

    smpl.save_obj(verts[0].cpu().numpy(), './mesh.obj')

    rpose = reflect_pose(pose)
    rpose_input = torch.tensor(np.array([rpose])).float().to(device)

    verts, j, r = smpl(shape_input, rpose_input, get_skin=True)
