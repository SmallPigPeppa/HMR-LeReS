...
class HMRLeReS(pl.LightningModule):
    ...
    def training_step(self, batch, batch_index):
        ...
        # 针对可能的非正方形图像进行调整
        img_width, img_height = img_size  # 假设img_size是一个包含两个元素的元组，分别为宽度和高度
        in_img_x = (gt_kpts_2d[:, :, 0] >= 0) & (gt_kpts_2d[:, :, 0] < img_width)
        in_img_y = (gt_kpts_2d[:, :, 1] >= 0) & (gt_kpts_2d[:, :, 1] < img_height)
