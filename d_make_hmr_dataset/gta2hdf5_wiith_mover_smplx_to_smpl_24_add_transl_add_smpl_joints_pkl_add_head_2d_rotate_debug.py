...
def normalize_rotation_vectors(rotation_vectors):
    ...
    rotation_vectors[np.linalg.norm(rotation_vectors, axis=1) < 0] = -rotation_vectors[
        np.linalg.norm(rotation_vectors, axis=1) < 0]

    # Normalize the rotation vectors
    rotation_vectors = rotation_vectors / np.expand_dims(np.linalg.norm(rotation_vectors, axis=1), axis=1)
...
if __name__=='__main__':
    ...
    # step4:: transfer kpts3d joints3d to 2d
    ...
    # step5: get smplx mesh pose and shape
    smpl_poses_matrix = []
    smpl_shapes = []
    smpl_transl = []
