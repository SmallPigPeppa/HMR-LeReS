...
def main(index):
    # index = 559
    out_path = 'leres-vis-out-all-top'
    ...
    pcd_r = pcd.get_rotation_matrix_from_xyz((np.pi * 0.5, np.pi * 0, np.pi * 0))
    pcd_t = np.array([0, 0, 0]).reshape([3, 1])
    pcd_t = all_t[index].reshape([3, 1])
    pcd_pose = np.hstack([pcd_r, pcd_t])
    pcd_pose = np.vstack([pcd_pose, [0, 0, 0, 1]])

    scene.add(mesh_human, 'mesh-human', pose=pcd_pose)
    ...
    cam_add_t = np.array([0, 0, -15]).reshape([3, 1])
    cam_t = avg_t.reshape([3, 1])
    cam_t=cam_t+cam_add_t
...
if __name__=="__main__":
    data_path = 'C:\\Users\\90532\\Desktop\\Datasets\\HMR-LeReS\\2020-06-11-10-06-48'
    info_npz = np.load(os.path.join(data_path, 'info_frames.npz'))
    world2cam = np.transpose(info_npz['world2cam_trans'], (0, 2, 1))
    cam2world = np.linalg.inv(world2cam)
    all_t = cam2world[:,:3, 3]
    avg_t = np.average(all_t, axis=0)
