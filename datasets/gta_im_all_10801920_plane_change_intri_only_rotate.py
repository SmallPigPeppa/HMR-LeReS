...
def normalize_rotation_vectors(rotation_vector):
    # Reshape the rotation_vector from (72,) to (24, 3)
    rotation_vector = rotation_vector.reshape(-1, 3)

    # Compute the norms (magnitudes) of the vectors
    norms = np.linalg.norm(rotation_vector, axis=1, keepdims=True)

    # Normalize angles to the range [-pi, pi]
    norms = ((norms + np.pi) % (2 * np.pi)) - np.pi

    # Compute the unit vectors
    unit_vectors = rotation_vector / np.clip(norms, a_min=1e-8, a_max=None)

    # Recompute the rotation vectors with the normalized angles
    normalized_rotation_vector = unit_vectors * norms

    # Reshape the normalized_rotation_vector back to (72,)
    normalized_rotation_vector = normalized_rotation_vector.reshape(-1)

    return normalized_rotation_vector
