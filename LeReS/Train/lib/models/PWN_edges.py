...
def edgeGuidedSampling(inputs, targets, edges_img, thetas_img, masks, h, w):
    ...
    distance_matrix = torch.randint(3, 20, (4,sample_num)).to(inputs.device)
...
class EdgeguidedNormalRegressionLoss(pl.LightningModule):
    def __init__(self, point_pairs=10000, cos_theta1=0.3, cos_theta2=0.95, cos_theta3=0.5, cos_theta4=0.86, mask_value=-1e-8, max_threshold=10.1):
        ...
        self.kernel = torch.tensor(np.array([[1, 1, 1], [1, 1, 1], [1, 1, 1]], dtype=np.float32), requires_grad=False)[None, None, :, :]

    def scale_shift_pred_depth(self, pred, gt):
        b, c, h, w = pred.shape
        mask = (gt > self.mask_value) & (gt < self.max_threshold)  # [b, c, h, w]
        EPS = 1e-6 * torch.eye(2, dtype=pred.dtype, device=self.device)
        scale_shift_batch = []
        ones_img = torch.ones((1, h, w), dtype=pred.dtype, device=self.device)
        for i in range(b):
            mask_i = mask[i, ...]
    ...
    def forward(self, pred_depths, gt_depths, images, focal_length):
        ...
        edges_mask_dilate = torch.clamp(torch.nn.functional.conv2d(edges_depth_mask.float(), self.kernel.to(self.device), padding=(1, 1)), 0,
                                       1).bool()
...
if __name__ == '__main__':
    ...
    gt_depth = torch.tensor(np.asarray(gt_depth, np.float32))
    pred_depth = torch.tensor(np.asarray(pred_depth, np.float32))
