...
class HMRLeReS(pl.LightningModule):
    def __init__(self):
        ...
        self.depth_regression_loss = DepthRegressionLoss(min_threshold=self.depth_min_threshold, max_threshold=self.depth_max_threshold)
        self.pwn_edge_loss = EdgeguidedNormalRegressionLoss(min_threshold=self.depth_min_threshold, max_threshold=self.depth_max_threshold)
        self.msg_loss = MultiScaleGradLoss(scale=4, min_threshold=self.depth_min_threshold, max_threshold=self.depth_max_threshold)
        self.edge_ranking_loss = EdgeguidedRankingLoss(min_threshold=self.depth_min_threshold, max_threshold=self.depth_max_threshold)
    ...
    def share_step(self, batch):
        ...
        kpts_verts_metrics = val_depth(predict_depth,gt_depth,min_threshold=self.depth_min_threshold,max_threshold=self.depth_max_threshold)
        depths_metrics = val_kpts_verts(pred_kpts_3d, gt_kpts_3d, pred_verts, gt_verts, pck_threshold=self.pck_threshold)
