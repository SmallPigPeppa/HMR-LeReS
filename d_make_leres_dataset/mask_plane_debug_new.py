...
#
#     # 将深度图转换为 32 位浮点数
#     depth_img_32f = depth_img.astype(np.float32)
#
#     # 使用双边滤波器对深度图进行平滑处理
#     depth_smooth = cv2.bilateralFilter(depth_img_32f, 9, 75, 75)
#
#     dX = cv2.Sobel(X, cv2.CV_64F, 1, 0, ksize=5)
#     dY = cv2.Sobel(Y, cv2.CV_64F, 0, 1, ksize=5)
#     dZ = np.ones_like(depth_img) * -1
...
# def compute_plane_masks(depth_img, intrinsic, eps=0.02, min_samples=5000):
#     normals = compute_normals(depth_img, intrinsic)
#     h, w, _ = normals.shape
#     normals_flat = normals.reshape(-1, 3)
#     mask = depth_img.reshape(-1) != 0
...
#     for i in range(1, num_planes + 1):
#         plane_mask_i = (plane_masks == i).astype(np.uint8)
#         num_labels, labels_im = cv2.connectedComponents(plane_mask_i)
#
#         # 更新平面掩码中的标签
#         plane_masks[labels_im > 0] = labels_im[labels_im > 0] + num_planes * (i - 1)
#
...
# def compute_plane_masks(depth_img,intrinsic, eps=0.012, min_samples=500):
#     normals = compute_normals(depth_img, intrinsic)
#     h, w, _ = normals.shape
#     normals_flat = normals.reshape(-1, 3)
#     mask = depth_img.reshape(-1) != 0
...
if __name__ == '__main__':
    ...
    for scene_dir in scene_dirs:
        ...
        for idx in tqdm(range(scence_samples)):
            ...
            plane_masks_i = compute_plane_masks(depth_i_resized, intrinsic_i_resized)
            min_cluster_size = 1000  # 设置最小簇大小阈值
            plane_masks_i = filter_clusters_by_size(plane_masks_i, min_cluster_size)
            # 确保平面的连通性
            # _, labels = cv2.connectedComponents(plane_masks_i.astype(np.uint8))
            # plane_masks_i = labels

            # normalized_plane_masks = plane_masks_i * (255 // np.max(plane_masks_i))
            ...
            # 标准化深度图和平面掩码以便显示
            normalized_depth_i = cv2.normalize(depth_i, None, alpha=0, beta=255, norm_type=cv2.NORM_MINMAX,
                                               dtype=cv2.CV_8U)
            # normalized_plane_masks = plane_masks_i * (255 // np.max(plane_masks_i))
            ...
            ax3.axis("off")
            if not os.path.exists(os.path.join(scene_dir, 'plane-debug-connect')):
                os.makedirs(os.path.join(scene_dir, 'plane-debug-connect'))

                # 修改：正确打开和关闭文件以保存结果
            with open(os.path.join(scene_dir, 'plane-debug-connect', '{:05d}'.format(idx) + '.png'), 'wb') as file:
