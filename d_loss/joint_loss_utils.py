...
def draw_color_map(depth, back_depth, dmin, dmax, save_path=None):
    ...
    im = ax1.imshow(depth[:360, :], cmap='gist_rainbow', alpha=.8, interpolation='nearest', vmin=dmin - 1e-2,
                    vmax=dmax + 1e-2)
    ...
    im = ax2.imshow(back_depth[:360, :], cmap='gist_rainbow', alpha=.8, interpolation='nearest', vmin=dmin - 1e-2,
                    vmax=dmax + 1e-2)
