...
from kpts_mapping.lsp import LSP_KEYPOINTS
import pickle
import os
import cv2
...
# print(mapping_list2)
kpts_lsp = numpy.zeros(shape=(len(kpts_gta), len(LSP_KEYPOINTS), 4))
for i in range(len(kpts_lsp)):
    for j in range(len(kpts_lsp[0])):
        if mapping_list2[j] != -1:
            kpts_lsp[i][j] = kpts_gta[i][mapping_list2[j]]
